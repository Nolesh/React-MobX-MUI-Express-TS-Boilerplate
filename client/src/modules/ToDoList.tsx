...
export default function () {
  ...
  const handleEditItemDialog = (item: TItemData) => {
    request(`${TODO_ROUTE}/${selectedIndex}`, "PATCH", item).then((res) =>
  ...
  const handleDelItemDialog = () => {
    request(`${TODO_ROUTE}/${selectedIndex}`, "DELETE").then((res) =>
      setItems(res.content)
    );
    handleCloseDialog();
  };
  ...
  return (
    <Container>
      ...
      >
        <ControlButton variant="contained" onClick={(e) => add()}>
          Add
        </ControlButton>
        <ControlButton
          variant="contained"
