...
const errorLogger = (error: MyErr, req: Request, res: Response, next: NextFunction) => {
    // logging errors    
    console.error(`Path: ${req.path}`, error.message, error.status ? `(${error.status})` : "", error.stack);
    next(error); // forward to next middleware
};

// error handler middleware
const errorResponder = (error: MyErr, req: Request, res: Response, next: NextFunction) => {
...
const throwError = (next: NextFunction, messageOrError: string | unknown, status?: number) => {
    try {
